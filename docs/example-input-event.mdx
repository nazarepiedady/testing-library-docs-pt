...
title: Evento de Entrada
sidebar_label: Evento de Entrada
---

> **Nota**
>
> Se quiseres simular um comportamento de digitação mais natural enquanto testas o teu componente, considere a biblioteca acompanhante [`user-event`](user-event/intro.mdx).
...
test('It should not allow letters to be inputted', () => {
  const {input} = setup()
  expect(input.value).toBe('') // vazia antes
  fireEvent.change(input, {target: {value: 'Good Day'}})
  expect(input.value).toBe('') // vazia depois
})

test('It should allow the $ to be deleted', () => {
  const {input} = setup()
  fireEvent.change(input, {target: {value: '23'}})
  expect(input.value).toBe('$23') // precisa de fazer uma mudança assim React regista "" como uma mudança
