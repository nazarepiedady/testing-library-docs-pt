...
The `userEvent.setup()` API applies these workarounds to the document and allows you to [configure](options.mdx) an "instance" of `user-event`. Methods on this instance share one input device state, e.g. which keys are pressed.
...
The [Clipboard API](https://developer.mozilla.org/en-US/docs/Web/API/Clipboard) is usually not available outside of secure context. To enable testing of workflows involving the clipboard, [`userEvent.setup()`](setup.mdx) replaces `window.navigator.clipboard` with a stub.
