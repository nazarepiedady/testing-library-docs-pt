...
Note that this API triggers pointer events and is therefore subject to [pointerEventsCheck](options.mdx#pointereventscheck).
...
> You should use [`keyboard()`](api-keyboard.mdx) if you want to just simulate pressing buttons on the keyboard.  
...
1. If `initialSelectionStart` is set, set the selection on the element. If `initialSelectionEnd` is not set, this results in a collapsed selection.
1. Type the given `text` per [`keyboard()`](api-keyboard.mdx).
1. Unless [`skipAutoClose`](options.mdx#skipautoclose) is `true`, release all pressed keys.
...
> Files that don't match an `accept` property will be automatically discarded, unless [`applyAccept`](options.mdx#applyaccept) is set to `false`.
