...
The `PointerTarget` props allows to describe the position of the pointer on the document. The `coords` you provide are applied as-is to the resulting [`MouseEvent`](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent) and can be omitted. The `target` should be the element receiving the pointer input in the browser. This is the topmost element that can receive pointer event at those coordinates.
