...
Note that the [Clipboard API](https://developer.mozilla.org/en-US/docs/Web/API/Clipboard) is usually not available outside of secure context. To enable testing of workflows involving the clipboard, [`userEvent.setup()`](setup.mdx) replaces `window.navigator.clipboard` with a stub.
...
If [`writeToClipboard`](options.mdx#writetoclipboard) is `true`, this will also write the data to the `Clipboard`.
